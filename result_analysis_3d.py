...
exclude = [
    'IOU_GT', 'IOU_PRED', 'eval', 'ouratio', 'Miss', 'over', 'under', 'overde', 'VI',
    'TP', 'FP', 'FN', 'Precision', 'Recall', 'F1', 'StopsH', 'RecordsH', 'UqH',
    'clus_points', 'clus_stops'
]

# Filter candidate metrics
candidate_metrics = [m for m in candidate_metrics if m not in exclude]

# Base prediction-real pairs
base_pairs = [
    ('PredH_15T', 'RealH_15T'),
    ('PredHC_15T', 'RealHC_15T'),
]

# Time resolutions to generate alternatives
time_resolutions = ['5T', '10T', '30T', '1H']

# Generate time-variant pairs
all_pairs = base_pairs.copy()
for a, b in base_pairs:
    if '_15T' in a or '_15T' in b:
        for tr in time_resolutions:
            ...
            all_pairs.append((new_a, new_b))

# Combine each (pred, real) pair with each candidate metric
combined_triplets = [(a, b, m) for (a, b) in all_pairs for m in candidate_metrics]
...
SET_SIZE = 3
for xval, yval, zval in combined_triplets:
    if len(set([xval,yval,zval])) < SET_SIZE:
        continue
    ...
    for person in reference.index:
        ...
        points = data_sorted[['x', 'y', 'z']].values
        ...
        scaler = MinMaxScaler()
        scaler.fit_transform(data_sorted.sort_values('iou')[['x', 'y', 'z', 'iou', 'pareto']])

        #IDEAL 1
        ideal = []
        for indi, val in enumerate([xval, yval, zval]):
            if 'clusT' in val or 'Pred' in val:
                ideal.append(1.0)
            elif 'Real' in val:
                if 'T' in val:
                    multiplier = 60 / int(''.join([w for w in val if w.isdigit()]))
                else:
                    multiplier = 1
                ideal.append(np.log2(hours_per_person[person]) * multiplier)
            else:
                ideal.append(data_sorted.iloc[:,indi].max())
        data_sorted['distance'] = np.sqrt((data_sorted.x - ideal[0]) ** 2 + (data_sorted.y - ideal[1]) ** 2 + (data_sorted.z - ideal[2]) ** 2)
