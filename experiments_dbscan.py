...
param_dict = {
    'eps': [50, 100, 200, 500],
    'min_samples': [1, 3],
    'stop_distance': [30, 50, 100, 300, 500],
    'stop_time': ['3T', '5T', '10T', '15T']
}
# param_dict = {
#     'eps': [100],
#     'min_samples': [3],
...
for x in param_combinations:
    ...
    for uid, udata in aggregated_df.groupby(level=0):
        ...
        udata.to_file(f'outputs\{uid}_{x.values()}.shp', driver='ESRI Shapefile')
...
with open('results_dbscan.pkl', 'wb') as f:
    pickle.dump(results, f)
with open('reference_dbscan.pkl', 'wb') as f:
